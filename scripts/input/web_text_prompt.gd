...
(function() {
	...
	window.symbiaTextPrompt = {
		...
		_focusInput: function(input) {
			// Android Chrome: focus synchronously on open, then retry; readonly trick helps GBoard.
			input.readOnly = true;
		...
		open: function(initial, placeholder, labelText) {
...
func open(line_edit: LineEdit, placeholder: String = "") -> void:
	...
	window.symbiaTextPrompt.open(line_edit.text, ph, label)
