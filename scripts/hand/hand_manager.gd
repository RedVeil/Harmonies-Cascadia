...
func add_card(card_data:CardData) -> void:
	...
	if matching_card_id == -1:
		...
		card_container.add_card(card_data, new_id, false)
		card_amount += 1
...
func _on_recycling_area_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			orchestrator.apply_recycle_card(-1, recycling_value, false)

func _on_recycling_area_mouse_entered() -> void:
	$RecyclingBin.modulate = Color.html("#cd4f3d")
	orchestrator.preview_recycle_card(-1, recycling_value, false)


func _on_recycling_area_mouse_exited() -> void:
	$RecyclingBin.modulate = Color.html("#e36451")
